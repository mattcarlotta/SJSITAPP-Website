...
describe("Token Resend Controller", () => {
  ...
  it("rejects requests where the token id is invalid", done => {
    app()
      .put("/api/tokens/resend-email/a01dc43483adb35b1ca678ea")
      .set("Cookie", cookie)
  ...
  it("rejects requests where the token has already been used", done => {
    app()
      .put(`/api/tokens/resend-email/${oldToken._id}`)
      .set("Cookie", cookie)
      .expect("Content-Type", /json/)
      .expect(400)
      .then(res => {
  ...
  it("accepts requests to resend a token", done => {
    app()
      .put(`/api/tokens/resend-email/${token._id}`)
      .set("Cookie", cookie)
      .expect("Content-Type", /json/)
      .expect(201)
      .then(res => {
        expect(res.body.message).toEqual(
          `An authorization token will be resent to ${newToken.authorizedEmail} shortly.`
        );
        done();
      });
  });
